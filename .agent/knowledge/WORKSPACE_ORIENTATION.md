...
- Plan 39 landed on 2026-06-06: modules now have Required/Core/Custom policy. Table View is required and always visible; Documents, Journal, Assets, and Web can be hidden from navigation but remain enabled; AI Chat, Prompt Studio, and Workflow Runner can be disabled. `modules:list` returns policy metadata, Settings exposes Core & Custom Plugins, rail/commands/browser preview honor enabled/visible state, Table View is the active fallback, and renderer module views/state are lazy-loaded or lazy-initialized where practical. Evidence lives in `implementation/screenshots/core-custom-modules-*-after-2026-06-06.png`.
- Plan 40 landed on 2026-06-06: Table View filters now support multi-kind custom selection with All/None/Invert, word-count min/max filtering, modified-date quick ranges, active filter chips, and `Showing X of Y documents` summaries. Legacy single `filterKind` snapshots migrate into the new state model. Evidence lives in `implementation/screenshots/table-filters-*-after-2026-06-06.png`.
- Plan 41 landed on 2026-06-06: Assets inspector now supports editable current-project roles and current-workspace document links through a typeahead, while source/technical metadata stays read-only. Evidence lives in `implementation/screenshots/asset-links-*-after-2026-06-06.png`.
